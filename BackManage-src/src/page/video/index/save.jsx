 ...
 * @Last Modified time: 2021-02-23 16:26:34
 */

import React from "react";
import MUtil from "util/mm.jsx";
import Video from "service/live-service.jsx";
...
class VideoSave extends React.Component {
  ...
  loadVideo() {
    ...
    if (this.state.id) {
      _product.getVideo(this.state.id).then(
        (res) => {
  ...
  onSubmit() {
    let product = {
      ...
      productCheckResult = _product.checkVideo(product);
    if (this.state.id) {
      product.id = this.state.id;
    }
    // 表单验证成功
    if (productCheckResult.status) {
      _product.saveVideo(product).then(
        (res) => {
          _mm.successTips(res);
  ...
  render() {
    return (
      <div id="page-wrapper">
        <PageTitle title={this.state.id ? "编辑短视频" : "新建短视频"} />
        <div className="form-horizontal">
          ...
          <div className="form-group">
            <label className="col-md-2 control-label">主播数</label>
            <div className="col-md-3">
              <div className="input-group">
                <input
                  ...
                  onChange={(e) => this.onValueChange(e)}
                />
                <span className="input-group-addon">人</span>
              </div>
            </div>
          </div>
          <div className="form-group">
